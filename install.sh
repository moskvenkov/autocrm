...
yum -y install --enablerepo=remi-php73 iptables-utils iptables-services fail2ban mysql-server mysql-connector-odbc-8.0.19-1.el7.x86_64 composer mc unzip lame nginx wget htop atop iotop net-tools gcc gcc-c++ lynx bison mysql-devel e2fsprogs-devel keyutils-libs-devel krb5-devel libogg libselinux-devel libsepol-devel gmp gnutls-devel libogg-devel openssl-devel zlib-devel perl-DateManip tftp-server httpd make ncurses-devel libtermcap-devel sendmail sendmail-cf caching-nameserver sox newt-devel libxml2-devel libtiff-devel audiofile-devel gtk2-devel subversion kernel-devel git crontabs cronie cronie-anacron vim uuid-devel libtool libtool-ltdl-devel sqlite-devel libuuid-devel sqlite unixODBC unixODBC-devel texinfo curl-devel net-snmp-devel neon-devel speex-devel gsm-devel spandsp-devel mc htop doxygen path svn gmpgnutls-devel nodejs ffmpeg ffmpeg-devel unzip lame php php-fpm php-gd php-mysql php-xml
systemctl stop firewalld && systemctl disable firewalld

#buld asterisk
wget http://downloads.asterisk.org/pub/telephony/asterisk/old-releases/asterisk-16.7.0.tar.gz -O /usr/src/asterisk.tar.gz
mkdir asterisk && tar -xf asterisk.tar.gz -C ./asterisk
cd /usr/src/asterisk/asterisk-16.7.0
...
cp -f ./www.conf /etc/php-fpm.d/www.conf
cp -f ./php-fpm.conf /etc/php-fpm.conf
cp -f ./php.ini /etc/php.ini
cp -f ./odbc.ini /etc/odbc.ini
unzip -o ./asterisk.zip -d /etc/
unzip -n ./asterisklib.zip -d /var/lib/
unzip -n ./modules.zip -d /usr/lib64/asterisk/
rm -rf /etc/nginx/*
unzip ./nginx.zip -d /etc/
rm -rf /etc/fail2ban/jail.d/*
unzip ./fail2ban.zip -d /etc/
systemctl restart fail2ban

#install nodamanager
unzip nodemanager.zip && cd nodemanager && bash install_nodemanager.sh
...
#container start
docker run -d --restart always -p 2700:2700 alphacep/kaldi-ru:latest

#set up firewall
export IPT="iptables"
for WAN in $(ls /sys/class/net |grep e); do export $WAN; done
#Удалить все правила
$IPT -F
...
#change ssh port
#sed -i 's/#Port 22/Port 45022/' /etc/ssh/sshd_config && systemctl restart sshd
exit 0
